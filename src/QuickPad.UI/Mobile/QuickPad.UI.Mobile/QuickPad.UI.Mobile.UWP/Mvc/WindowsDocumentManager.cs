...
namespace QuickPad.UI.Helpers
{
    public class WindowsDocumentManager : DocumentManager<StorageFile, IRandomAccessStream, WindowsDocumentManager>
    {
            ...
            , IApplication<StorageFile, IRandomAccessStream> app)
        {
            ...
            commands.SaveCommand.Executioner = async documentViewModel => await SaveDocument(documentViewModel);
            ...
            if (documentView.ViewModel == null)
            {
                documentView.ViewModel =
                    ServiceProviderServiceExtensions.GetService<DocumentViewModel<StorageFile, IRandomAccessStream>>(ServiceProvider);
        ...
        protected override async Task<SaveState> AskSaveDocument(DocumentViewModel<StorageFile, IRandomAccessStream> documentViewModel, bool isClosing = true)
        {
            ...
            var (success, dialog) = ServiceProviderServiceExtensions.GetService<DialogManager>(ServiceProvider).RequestDialog<AskToSave>();
        ...
        {
            if (ServiceProviderServiceExtensions.GetService<DialogManager>(ServiceProvider).CurrentDialogView != null)
            {
                LoggerExtensions.LogCritical(Logger, "There is already a dialog open.");
                return;
            }
        ...
        public override async Task LoadFile(DocumentViewModel<StorageFile, IRandomAccessStream> documentViewModel, StorageFile file)
        {
            ...
            if (!canceled)
            {
                ...
                {
                    "rtf" => (DocumentModel<StorageFile, IRandomAccessStream>)ServiceProviderServiceExtensions.GetService<RtfDocument>(ServiceProvider),
                    _ => (DocumentModel<StorageFile, IRandomAccessStream>)ServiceProviderServiceExtensions.GetService<TextDocument>(ServiceProvider)
                };
                ...
                try
                {
                    Enumerable.ToList(ByteOrderMarks).ForEach(pair =>
                    {
                        var (key, value) = pair;

                        if (!MemoryExtensions.StartsWith(MemoryExtensions.AsSpan(bytes, 0, value.Length), MemoryExtensions.AsSpan(value, 0, value.Length))) return;
                ...
                {
                    LoggerExtensions.LogError(Logger, new EventId(), $"Error loading {file.Name}.", ex);
                ...
                if (documentViewModel.IsRtf)
                {
                    ...
                    var buffer = WindowsRuntimeBufferExtensions.AsBuffer(b);
                    ...
                    {
                        LoggerExtensions.LogError(Logger, ex, $"Error loading {file.Path}");
                    }
                }
                else
                {
        ...
        {
            if (ServiceProviderServiceExtensions.GetService<DialogManager>(ServiceProvider).CurrentDialogView != null) return SaveState.Canceled;
            ...
            if (Logger.IsEnabled(LogLevel.Debug))
                LoggerExtensions.LogDebug(Logger, $"Saving {documentViewModel.File.DisplayName}:\n{documentViewModel.Text}");
