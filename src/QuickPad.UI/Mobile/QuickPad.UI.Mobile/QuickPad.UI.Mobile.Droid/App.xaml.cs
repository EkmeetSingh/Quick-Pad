...
using AndroidQuickPadCommands =
    QuickPad.Mvvm.Commands.AndroidQuickPadCommands;
...
namespace QuickPad.UI
{
    ...
    {
        private static AndroidSettingsViewModel _settings;
        public static ApplicationHost<StorageFile, IRandomAccessStream, AndroidDocumentManager> Host { get; set; }
        public static ApplicationController<StorageFile, IRandomAccessStream, AndroidDocumentManager> Controller => Host?.Controller;
        public static IServiceProvider ServiceProvider => Host?.Services;
        public IServiceProvider Services => Host?.Services;
        public static AndroidSettingsViewModel Settings => _settings ??= Host?.Services.GetService<AndroidSettingsViewModel>();
        public static RichEditBox RichEditBox { get; internal set; }

        public static AndroidQuickPadCommands Commands => ServiceProvider.GetService<AndroidQuickPadCommands>();
        ...
        public App()
        {
            ...
            AppDomainSetup setup =
                typeof(AppDomain).GetProperty("SetupInformationNoCopy", BindingFlags.NonPublic | BindingFlags.Instance)?
                    .GetValue(AppDomain.CurrentDomain) as AppDomainSetup;

            if (setup == null) throw new ApplicationException("Cannot get setup of AppDomain.");
            ...
            System.Diagnostics.Debug.WriteLine(AppContext.BaseDirectory);
                ...
                Host = builder.BuildApplicationHost<StorageFile, IRandomAccessStream, AndroidDocumentManager>();

            Logger = ServiceProvider.GetService<ILogger<App>>();

            this.InitializeComponent();

            Resources.Add("ToggleButtonRevealStyle", new Style());
            Resources.Add("SplitButtonRevealStyle", new Style());
            Resources.Add("ButtonRevealStyle", new Style());
            Resources.Add("SystemControlAcrylicWindowBrush", new SolidColorBrush());
        ...
        protected override void OnLaunched(LaunchActivatedEventArgs e)
        {
            ...
            if (mainPage == null)
            {
                ...
                if (!Resources.ContainsKey(nameof(AndroidQuickPadCommands))) Resources.Add(nameof(AndroidQuickPadCommands), ServiceProvider.GetService<IQuickPadCommands<StorageFile, IRandomAccessStream>>());
                if (!Resources.ContainsKey(nameof(AndroidSettingsViewModel))) Resources.Add(nameof(AndroidSettingsViewModel), ServiceProvider.GetService<AndroidSettingsViewModel>());
        ...
        protected override async void OnFileActivated(FileActivatedEventArgs args)
        {
            ...
            if (!Resources.ContainsKey(nameof(AndroidQuickPadCommands)))
                Resources.Add(nameof(AndroidQuickPadCommands), ServiceProvider.GetService<AndroidQuickPadCommands>());
            if (!Resources.ContainsKey(nameof(AndroidSettingsViewModel)))
                Resources.Add(nameof(AndroidSettingsViewModel), ServiceProvider.GetService<AndroidSettingsViewModel>());

            if (!Resources.ContainsKey("ToggleButtonRevealStyle"))
            {
                Resources.Add("ToggleButtonRevealStyle", new Style());
            }
        ...
        private async void MainPageOnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                var windowsDocumentManager = ServiceProvider.GetService<AndroidDocumentManager>();
        ...
        {
            return (Task<TResult>)CoreApplication.MainView.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () => action?.Invoke()).AsTask();
        }
